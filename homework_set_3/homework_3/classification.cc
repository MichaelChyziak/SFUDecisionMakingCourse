...
//
// Program description:
// 	Reads wine data and clusters the data into 3 clusters. Then it uses perceptrons to create decision boundaries between each cluster. Then it asks a user to input some 2D data and classifies the data.
// Required supplementary files:
// 	wine.data
// Got data set from here: 
//	http://archive.ics.uci.edu/ml/machine-learning-databases/wine/wine.data
// g++ classification.cc -lm -lglut -lGL -lGLU -o classification
...
void drawScreen() {
	...
	if (done_k_means == false) {
		...
		displayString(-0.5f, 0.85f, "Perceptron decision boundary of Wine Alcohol vs Wine Hue (epoch: " + to_string(epoch) + ")");
		displayString(-0.6f, 0.8f, "Perceptron decision boundary complete at epoch " + to_string(epoch) + ". Enter user data in terminal to get cluster classification.");
	}
    ...
    if (done_perceptron == true) {
    	getUserInput();
    }
}

void getUserInput() {
	string user_input_alcohol;
	string user_input_hue;
	float weight_class_1_value;
	float weight_class_3_value;
	string cluster_sort;
	...
	while(run_program == true) {
		...
		// Get user input
		printf("Enter the wine alcohol value: ");
		cin >> user_input_alcohol;
		printf("Enter the wine hue value: ");
		cin >> user_input_hue;

		// Check if input is valid
		if (strtof(user_input_alcohol.c_str(), NULL) != 0 && strtof(user_input_hue.c_str(), NULL) != 0) {
			weight_class_1_value = strtof(user_input_alcohol.c_str(), NULL) * weight_class_1[0] + strtof(user_input_hue.c_str(), NULL) * weight_class_1[1] + constant_bias * weight_class_1[2];
			weight_class_3_value = strtof(user_input_alcohol.c_str(), NULL) * weight_class_3[0] + strtof(user_input_hue.c_str(), NULL) * weight_class_3[1] + constant_bias * weight_class_3[2];

			if (weight_class_1_value > 0 && weight_class_3_value > 0) {
				cluster_sort = "cluster 1 or cluster 3";	
			}
			else if (weight_class_1_value > 0) {
				cluster_sort = "cluster 1";
			}
			else if (weight_class_3_value > 0) {
				cluster_sort = "cluster 3";
			} 
			else {
				cluster_sort = "cluster 2";
			}
			
			cout << "The data point alcohol = " + user_input_alcohol + ", hue = " + user_input_hue + ", can belong in " + cluster_sort + ".\n"; 
			//printf("The data point alcohol = " + user_input_alcohol.c_str() + ", hue = " + user_input_hue.c_str() + ", can belong in " + cluster_sort + ".\n");
		}
		else {
